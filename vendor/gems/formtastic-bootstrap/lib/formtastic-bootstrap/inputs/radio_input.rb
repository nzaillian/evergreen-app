module FormtasticBootstrap
  module Inputs
    class RadioInput < Formtastic::Inputs::RadioInput
      ...
      def to_html
        if hint?
          classes = options[:hint_class] ? options[:hint_class] : "help-block"
          hint_markup = template.content_tag(:span, hint_html, class: classes)
          controls_markup = hint_markup.html_safe
        else
          controls_markup = ""
        end

        controls_markup +=  collection.map { |choice| choice_html(choice) }.join("\n").html_safe

        control_group_wrapping do
          control_label_html <<
          controls_wrapping do
            controls_markup.html_safe
          end
        end
      end
